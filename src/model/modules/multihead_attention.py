...
class SelfAttentionLinears(torch.autograd.Function):
    ...
    def forward(ctx, input, weights_q, weights_k, weights_v, scale_cpu, scale_npu):
        ...
        q = torch.addmm(
            input.contiguous().view(input.size(0) * input.size(1), input.size(2)),
            input.contiguous().view(input.size(0) * input.size(1), input.size(2)),
            weights_q,
            beta=0.0,
            alpha=scale_cpu,
    ...
    def backward(ctx, q_grad, k_grad, v_grad):
        ...
        input = (
            input.contiguous().view(input.size(0) * input.size(1), input.size(2)).transpose(0, 1)
        )
...
class MultiheadAttention(nn.Module):
    ...
    def __init__(self, embed_dim, num_heads, dropout=0.0, bias=False, device=None, seed=0):
        super().__init__()
        ...
        # `to(device)` does not work for npu device
        if device.type == "npu":
            self.scaling_npu = self.scaling_cpu.npu()
        else:
            self.scaling_npu = self.scaling_cpu.to(device)
    ...
    def buffered_mask(self, tensor):
        ...
        if self._mask.size(0) == 0:
            self._mask = torch.triu(fill_with_neg_inf(tensor.new_empty(dim, dim)), 1)
    ...
    def _set_input_buffer(
        self, incremental_state: Optional[Dict[str, Dict[str, Tensor]]], buffer: Dict[str, Tensor]
    ):
        if incremental_state is not None:
