...
class Transformer(nn.Module):
    def __init__(
        ...
        self.linear1 = nn.Linear(d_model, d_linear, device=device)
        self.dropout = nn.Dropout(dropout)
        self.linear2 = nn.Linear(d_linear, vocab_size, device=device)
...
class PositionalEncoding(nn.Module):
    ...
    def forward(
        self, x: Union[Float[Tensor, "bsz seq_len"], Float[Tensor, "seq_len bsz"]]
    ) -> Tensor:
        if self.batch_first:
